...
--tblMember 테이블의 phone 필드의 크기 변경
...
--3번 레코드의 주소를 수정(광주시 북구 용봉동 → 서울시 강서구 화곡동)
update tblMember
...
select max(age) from tblMember;  --age의 최대값
select min(age) from tblMember;  --age의 최소값
select age + 2 from tblMember;  --입력된 age 값에 2를 더한 후 출력
