...
#Image Folder Path
path_folder = "/home/pi/Python/Pictures"
#Width of slideshow
slideshow_width = 1024
#Height of slideshow
slideshow_height = 768
#Transition time slideshow
slideshow_trasnition_time = 1
#Image stable time
...
def load_img(pathImageRead, resizeWidth, resizeHeight):     
    ...
    _img_input = cv2.imread(pathImageRead,cv2.IMREAD_COLOR)
...
def imgShow():    
    ...
    for imge_path in img_path_list:
        ...
        if img_one is None:
            ...
            #mine
            _img_input = cv2.imread(imge_path,cv2.IMREAD_COLOR)
            img_one = cv2.resize(_img_input, dsize=(slideshow_width, slideshow_height), interpolation=cv2.INTER_AREA)
